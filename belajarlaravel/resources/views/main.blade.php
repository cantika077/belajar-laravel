...
<body class="bg-white text-gray-900">
    ...
    <nav class="flex justify-between items-center px-10 py-6 max-w-7xl mx-auto border-b border-gray-50">
        ...
        <div class="flex items-center gap-8 font-medium text-gray-600 text-sm">
            ...
            <a href="{{ route('artikel') }}" class="hover:text-black transition">Artikel</a>
            <a href="{{ route('kontak') }}" class="hover:text-black transition">Kontak</a>
            <a href="{{ route('profil.indonesia') }}" class="hover:text-black transition">Profil</a>
            ...
            @auth
                ...
                <form method="POST" action="{{ route('logout') }}" class="inline">
                    @csrf
                    <button type="submit" class="bg-red-500 text-white px-5 py-2 rounded-md font-semibold hover:bg-red-600 transition">
                        Logout
                    </button>
                </form>
            @else
                {{-- Belum login: tampilkan tombol Login --}}
                <a href="{{ route('login') }}" class="bg-[#1D4ED8] text-white px-5 py-2 rounded-md font-semibold hover:bg-blue-700 transition">
                    Login
                </a>
            @endauth
        </div>
