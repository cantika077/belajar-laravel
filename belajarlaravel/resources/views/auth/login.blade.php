...
<body class="bg-slate-900 min-h-screen flex items-center justify-center text-white">
    ...
    <div class="relative z-10 w-full max-w-md px-6">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-slate-400 text-sm mt-2">Masuk ke akun kamu</p>
        </div>
        ...
        <div class="bg-slate-800/40 backdrop-blur-md border border-slate-700/60 rounded-2xl p-8 shadow-2xl">
            ...
            @if (session('status'))
                <div class="mb-4 p-3 bg-green-500/10 border border-green-500/30 rounded-xl text-green-400 text-xs font-semibold">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('login') }}">
                ...
                <div class="flex items-center justify-between mb-6">
                    <label class="flex items-center gap-2 text-xs text-slate-400 cursor-pointer">
                        <input type="checkbox" name="remember" class="rounded border-slate-600 bg-slate-800 text-blue-500">
                        Remember me
                    </label>
            ...
            <p class="text-center text-xs text-slate-400 mt-6">
                Belum punya akun? 
                <a href="{{ route('register') }}" class="text-blue-400 font-bold hover:text-blue-300 transition">
                    Daftar sekarang
                </a>
            </p>
